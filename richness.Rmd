...
```
Plot-level plant species richness significantly differed among habitat clusters (ANOVA:F=15.79, p < 0.0001, DF = 402)

```{r}
plant_aov = alpha_raw_anova_by_cluster(plant_presence, insect_plants_plots)
plant_cld_alpha = tukey_groups = agricolae::HSD.test(plant_aov, trt = "cluster_id")
...
```{r}
plot_alpha_richness(plant_richness, plant_cld_alpha, fill_scale(append_insect_plant_palette(insect_plant_clusters)))
...
```{r}
plot_alpha_richness(bird_richness, bird_cld_alpha, fill_scale(append_bird_palette(bird_clusters, append_insect_plant_palette(insect_plant_clusters))))
...
insect_c_adonis_res
```
Let's check insects (species only) against the bird clusters
```{r}
...
plant_adonis_res
```
Let's check plants against the bird clusters
```{r}
...
Compare with insects (species only) using bird clusters
```{r}
beta.multi(beta_core(insect_b_presence, bird_plots)) 
```
...
So, most of the compositional variation among sites is due to species replacement rather than sites being species-poor subsets of richer ones.
...
```{r}
beta.multi(beta_core(plant_presence, bird_plots)) 
```
