...
```{r}
alpha_anovo_model_by_clusters = function(comm, meta) {
  alpha_plot = specnumber(comm)
  meta$alpha = alpha_plot
  lm(alpha ~ cluster_id, data = meta)
}

anova_model = function(model) {
  aov(model)
}

alpha_raw_anova_by_cluster = function(comm, meta) {
  model = alpha_anovo_model_by_clusters(comm, meta)
  anova_model(model)
}
...
```{r}
spatial_alpha_anovo_model_by_clusters = function(comm, meta) {
  alpha_plot = specnumber(comm)
  meta$alpha = alpha_plot
  meta$cluster_id = factor(meta$cluster_id)
  meta$cluster_id = relevel(meta$cluster_id, ref = "impervious / hard-standing")
  
  meta$x = coords[,1]
  meta$y = coords[,2]
  gam(alpha ~ cluster_id + s(x, y, bs = "gp"), data = meta)
}

spatial_alpha_anovo_model_by_clusters(insect_a_presence, insect_plants_plots) %>% summary()
```
...
```{r}
richness_by_cluster = function(comm, meta, clusters)
  ...
  left_join(clusters, by = c('cluster_id' = 'cluster_name')) %>%
  arrange(avg_ndvi) %>%
  dplyr::select(cluster_id, mean_alpha, sd_alpha, n_plots, gamma, beta_whittaker)
```
...
plot_alpha_richness = function(richess_df, cld, fill_scale) {
  plot_data = as.data.frame(cld) %>%
    dplyr::transmute(
      ...
      mean_alpha = emmean,
      se_alpha   = SE,
      groups     = trimws(.group)   # .group often has trailing spaces
    )
...
```{r}
richness_by_cluster(insect_b_presence, bird_plots, bird_clusters) %>% summarise(mean_bw = mean(beta_whittaker))
richness_by_cluster(insect_b_presence, insect_plants_plots, insect_plant_clusters) %>% summarise(mean_bw = mean(beta_whittaker))
```

```{r}
insect_b_richness = richness_by_cluster(insect_b_presence, insect_plants_plots, insect_plant_clusters)
insect_b_richness %>% mutate(across(where(is.numeric), round, digits=2))
```

```{r}
spatial_alpha_anovo_model_by_clusters(insect_b_presence, insect_plants_plots) %>% summary()
```

```{r}
insect_b_emm_clusters = emmeans(spatial_alpha_anovo_model_by_clusters(insect_b_presence, insect_plants_plots), ~cluster_id)
insect_b_cld_clusters = multcomp::cld(insect_b_emm_clusters, adjust = "tukey")
insect_b_cld_clusters
```

**Richness amongst 120 x 120 m habitat clusters**
```{r}
insect_b2_richness = richness_by_cluster(insect_b_presence, bird_plots, bird_clusters)
insect_b2_richness %>% mutate(across(where(is.numeric), round, digits=2))
```

```{r}
spatial_alpha_anovo_model_by_clusters(insect_b_presence, bird_plots) %>% summary()
```


```{r}
insect_b2_emm_clusters = emmeans(spatial_alpha_anovo_model_by_clusters(insect_b_presence, bird_plots), ~cluster_id)
insect_b2_cld_clusters = multcomp::cld(insect_b2_emm_clusters, adjust = "tukey")
insect_b2_cld_clusters
```

```{r}
plot_alpha_richness(insect_b2_richness, insect_b2_cld_clusters, fill_scale(bird_clusters_colour))
ggsave('output/figure_om_insect_groups_120m_plots.jpg', height = 2000, width = 2000, units = 'px') 
```

**With any_butterfly, any_hoverfly, etc.**
```{r}
insect_a_richness = richness_by_cluster(insect_a_presence, insect_plants_plots, insect_plant_clusters)
insect_a_richness
```
...
```{r}
insect_c_richness = richness_by_cluster(insect_c_presence, insect_plants_plots, insect_plant_clusters)
insect_c_richness
```
...
Which set of clusters give more compositionally diverse plots for bird species?
```{r}
richness_by_cluster(plant_presence, bird_plots, bird_clusters) %>% summarise(mean_bw = mean(beta_whittaker))
richness_by_cluster(plant_presence, insect_plants_plots, insect_plant_clusters) %>% summarise(mean_bw = mean(beta_whittaker))
```

**20 x 20 m Clusters**
```{r}
plant_richness = richness_by_cluster(plant_presence, insect_plants_plots, insect_plant_clusters)
plant_richness %>% mutate(across(where(is.numeric), round, digits=2))
```

```{r}
spatial_alpha_anovo_model_by_clusters(plant_presence, insect_plants_plots) %>% summary()
```

```{r}
plant_emm_clusters = emmeans(spatial_alpha_anovo_model_by_clusters(plant_presence, insect_plants_plots), ~cluster_id)
plant_cld_clusters = multcomp::cld(plant_emm_clusters, adjust = "tukey")
plant_cld_clusters
```

```{r}
plot_alpha_richness(plant_richness, plant_cld_clusters, fill_scale(insect_plant_clusters_colour))
ggsave('output/figure_om_plant_groups_20m_plots.jpg', height = 2000, width = 2000, units = 'px')
```


**120 x 201 m Clusters**
```{r}
plant2_richness = richness_by_cluster(plant_presence, bird_plots, bird_clusters)
plant2_richness %>% mutate(across(where(is.numeric), round, digits=2))
```

```{r}
spatial_alpha_anovo_model_by_clusters(plant_presence, bird_plots) %>% summary()
```

```{r}
plant2_emm_clusters = emmeans(spatial_alpha_anovo_model_by_clusters(plant_presence, bird_plots), ~cluster_id)
plant2_cld_clusters = multcomp::cld(plant2_emm_clusters, adjust = "tukey")
plant2_cld_clusters
```

```{r}
plot_alpha_richness(plant2_richness, plant2_cld_clusters, fill_scale(bird_clusters_colour))
ggsave('output/figure_om_plant_groups_120m_plots.jpg', height = 2000, width = 2000, units = 'px')
```

## Birds
Which set of clusters give more compositionally diverse plots for bird species?
```{r}
richness_by_cluster(bird_presence, bird_plots, bird_clusters) %>% summarise(mean_bw = mean(beta_whittaker))
richness_by_cluster(bird_presence, insect_plants_plots, insect_plant_clusters) %>% summarise(mean_bw = mean(beta_whittaker))
```

### 20 x 20 m Clusters
```{r}
bird2_richness = richness_by_cluster(bird_presence, insect_plants_plots, insect_plant_clusters)
bird2_richness %>% mutate(across(where(is.numeric), round, digits=2))
```

```{r}
spatial_alpha_anovo_model_by_clusters(bird_presence, insect_plants_plots) %>% summary()
```

```{r}
bird2_emm_clusters = emmeans(spatial_alpha_anovo_model_by_clusters(bird_presence, insect_plants_plots), ~cluster_id)
bird2_cld_clusters = multcomp::cld(bird2_emm_clusters, adjust = "tukey")
bird2_cld_clusters
```

### 120 x 120 m Clusters
```{r}
bird_richness = richness_by_cluster(bird_presence, bird_plots, bird_clusters)
bird_richness %>% mutate(across(where(is.numeric), round, digits=2))
```

```{r}
spatial_alpha_anovo_model_by_clusters(bird_presence, bird_plots) %>% summary()
```

```{r}
bird_emm_clusters = emmeans(spatial_alpha_anovo_model_by_clusters(bird_presence, bird_plots), ~cluster_id)
bird_cld_clusters = multcomp::cld(bird_emm_clusters, adjust = "tukey")
bird_cld_clusters
```

```{r}
plot_alpha_richness(bird_richness, bird_cld_clusters, fill_scale(bird_clusters_colour))
ggsave('output/figure_om_bird_groups_120m_plots.jpg', height = 2000, width = 2000, units = 'px')
```
...
ordination = function(dist_jac, meta, colour_scale, taxa, cluster, exclusions = c()) {
  ...
  ggplot(ord_data, aes(x = NMDS1, y = NMDS2, colour = cluster_id)) +
    ...
    theme_minimal() +
    labs(
      title = paste("Clustered", cluster, "habitat"),
...
nmds_part_theme = function(include_legend_position = T, include_x_axis = T, include_y_axis = T) {
  legend_position = ifelse(include_legend_position, 'right', 'none')
  y_axis_text = axis_text(include_y_axis)
  x_axis_text = axis_text(include_x_axis)
...
legend2 = ggpubr::get_legend(insect_b2_ord)

row_nmds_1 = cowplot::plot_grid(
  insect_b_ord + nmds_part_theme(F, F, T),
  insect_b2_ord + nmds_part_theme(F, F, F)
)

row_nmds_2 = cowplot::plot_grid(
  plant_ord + nmds_part_theme(F, F, T),
  plant2_ord + nmds_part_theme(F, F, F)
)

row_nmds_3 = cowplot::plot_grid(
  bird_ord + nmds_part_theme(F, T, T),
  bird2_ord + nmds_part_theme(F, T, F)
)

row_nmds_4 = cowplot::plot_grid(
  legend1,
  legend2
)

jpeg('output/figure_3_nmds.jpg', width = 2100, height = 2700, units = "px", res = 300)
cowplot::plot_grid(
  cowplot::ggdraw() + cowplot::draw_label("NMDS of insect species composition", size = 16, hjust = 0.9),
  row_nmds_1,
  cowplot::ggdraw() + cowplot::draw_label("NMDS of plant species composition", size = 16, hjust = 0.9),
  row_nmds_2,
  cowplot::ggdraw() + cowplot::draw_label("NMDS of bird species composition", size = 16, hjust = 0.9),
  row_nmds_3,
  row_nmds_4,
  ncol = 1,
  rel_heights = c(1, 10, 1, 10, 1, 10, 12))
dev.off()
```
...
```{r}
scale_fill_turnover = function(..., fixed_max_turnover) {
  scale_fill_viridis_c(
    option = "C",          # "viridis" option; colour-blind-friendly
    limits = c(0, fixed_max_turnover),    # fixed min / max
...
turnover = function(beta_pair, species, cluster, cluster_names, fixed_max_turnover = 0.2) {
  ...
  turnover_df =
    ...
    left_join(cluster_names, by = c('cluster_from' = 'cluster_name')) %>% 
    rename('cluster_from_ndvi' = avg_ndvi) %>%
    left_join(cluster_names, by = c('cluster_to' = 'cluster_name')) %>% 
    rename('cluster_to_ndvi' = avg_ndvi) %>%
    mutate(
      # left axis ordered by insect_ndvi
      cluster_from =
        fct_reorder(cluster_from, cluster_from_ndvi),

      # right axis ordered by bird_ndvi
      cluster_to =
        fct_reorder(cluster_to, desc(cluster_to_ndvi))
    )
  
  ggplot(turnover_df, aes(x = cluster_from, y = cluster_to, fill = turnover)) +
    geom_tile() +
    theme_minimal() +
    scale_fill_turnover(fixed_max_turnover = fixed_max_turnover) +
    labs(
      title = paste(cluster, "clustered habitat"),
      x = "Cluster",
      y = "Cluster",
      fill = "Turnover"
    ) + 
...
```{r}
insect_b_turnover = turnover(insect_b_beta_pair, 'insect', '20 x 20 m', insect_plant_clusters, fixed_max_turnover = 0.7)
insect_b_turnover
```
...
```{r}
insect_b2_turnover = turnover(insect_b2_beta_pair, 'insect', '120 x 120 m', bird_clusters, fixed_max_turnover = 0.7)
insect_b2_turnover
```
...
```{r}
plant_turnover = turnover(plant_beta_pair, 'plant', '20 x 20 m', insect_plant_clusters)
plant_turnover
```
...
```{r}
plant2_turnover = turnover(plant2_beta_pair, 'plant', '120 x 120 m', bird_clusters)
plant2_turnover
```
...
```{r}
bird_turnover = turnover(bird_beta_pair, 'bird', '20 x 20 m', insect_plant_clusters)
bird_turnover
```
...
```{r}
bird2_turnover = turnover(bird2_beta_pair, 'bird', '120 x 120 m', bird_clusters)
bird2_turnover
```
...
jpeg('output/figure_4_turnover.jpg', width = 2100, height = 2700, units = "px", res = 300)
row_turnover1 = cowplot::plot_grid(
  
  insect_b_turnover + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.text.y = label_size, legend.position = 'none', plot.title = title_size), 
  insect_b2_turnover + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank(), plot.title = title_size) + ylab(''),
  ncol = 2,
  rel_widths = rel_widths
)
  
row_turnover2 = cowplot::plot_grid(
  
  plant_turnover + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.text.y = label_size, legend.position = 'none', plot.title = title_size), 
  plant2_turnover + theme(axis.title.x = element_blank(), axis.text.x = element_blank(), axis.text.y = element_blank(), plot.title = title_size) + ylab(''),
  ncol = 2,
  rel_widths = rel_widths
)

row_turnover3 = cowplot::plot_grid(
  
  bird_turnover + theme(axis.title.x = element_blank(), axis.text.x = label_size, axis.text.y = label_size, legend.position = 'none', plot.title = title_size), 
  bird2_turnover + theme(axis.title.x = element_blank(), axis.text.x = label_size, axis.text.y = element_blank(), plot.title = title_size) + ylab(''),
  ncol = 2,
  rel_widths = rel_widths
)

cowplot::plot_grid(
  cowplot::ggdraw() + cowplot::draw_label("Pairwise insect turnover", size = 16, hjust = 0.9),
  row_turnover1, 
  cowplot::ggdraw() + cowplot::draw_label("Pairwise plant turnover", size = 16, hjust = 0.9),
  row_turnover2,
  cowplot::ggdraw() + cowplot::draw_label("Pairwise bird turnover", size = 16, hjust = 0.9),
  row_turnover3,
  ncol = 1,
  rel_heights = c(1, 10, 1, 10, 1, 20))
dev.off()
```
...
```{r}
insect_a_turnover = turnover(insect_a_beta_pair, 'insect', '20 x 20 m', insect_plant_clusters)
insect_a_turnover
```
...
```{r}
insect_c_turnover = turnover(insect_c_beta_pair, 'insect', '20 x 20 m', insect_plant_clusters)
insect_c_turnover
```
